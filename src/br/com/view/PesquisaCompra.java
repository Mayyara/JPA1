 ...
 */
public class PesquisaCompra extends javax.swing.JFrame {

    private CompraTableModel model;

    public PesquisaCompra() {
        initComponents();
        List<Compra> lista = new ArrayList<Compra>();
        CompraController c = new CompraController();
        lista = c.listarCompra();
      
        model = new CompraTableModel(lista);
    ...
    private void botaoInserirActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoInserirActionPerformed
        JanelaCompra jc = new JanelaCompra(model);
        jc.setVisible(true);
    ...
    private void botaoRemoverActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoRemoverActionPerformed
        ...
        if (selecionado >= 0) {
            ...
            if (cc.excluir(i)){
            model.removeCompra(selecionado);
            }
    ...
    private void botaoEditarActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_botaoEditarActionPerformed
        ...
        if (selecionado >= 0) {
            Integer cdCompra = (Integer) model.getValueAt(selecionado, 0);
            JanelaAtualizarCompra ac = new JanelaAtualizarCompra(selecionado, cdCompra, model);
            ac.setVisible(true);
        } else {
    ...
    private void efetuaPesquisa() {
        String produto = campoPesquisa.getText();
        List<Compra> lista = new ArrayList<Compra>();
        CompraController c = new CompraController();
        lista = c.listarCompraByProduto(produto);
        model.limpar();

        
        model = new CompraTableModel(lista);

        //atribui o compra à tabela
        tabela.setModel(model);
    }
}
